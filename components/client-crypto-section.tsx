...
export default function ClientCryptoSection({
  userProfile,
  bankKey,
}: CryptoSectionProps) {
  ...
  const fetchCryptoBalances = async () => {
    if (!userProfile?.id || !bankKey) return;

    try {
      const response = await fetch(`/api/crypto-balances?userId=${userProfile.id}&bankKey=${bankKey}`);
  ...
  const fetchCryptoTransactions = async () => {
    ...
    try {
      const response = await fetch(`/api/crypto-transactions?userId=${userProfile.id}&bankKey=${bankKey}`);
  ...
  const submitTransfer = async () => {
    ...
    try {
      ...
      const gasFee = parseFloat(formData.gas_fee);
      const totalAmount = amount + gasFee;

      const currentBalance = cryptoBalances[
        `${formData.crypto_type.toLowerCase()}_balance` as keyof CryptoBalances
      ];

      if (totalAmount > currentBalance) {
        toast.error(`Insufficient ${formData.crypto_type} balance for this transfer`);
        return;
      }
      ...
      const response = await fetch('/api/crypto-transactions/create', {
        ...
        body: JSON.stringify({
          ...
          status: 'Pending',
          wallet_address: formData.recipient_address,
  ...
  const gasFee = parseFloat(formData.gas_fee) || 0;
  const totalAmount = amount + gasFee;
  ...
  const activeNotifications = cryptoTransactions.filter(
    (tx) =>
      ...
      (tx.status.toLowerCase() === "pending" || tx.status.toLowerCase() === "completed")
  );
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="p-6 pt-4 space-y-6 relative min-h-full">
                ...
                <div className="animate-in slide-in-from-right duration-300 pointer-events-auto">
                  <Alert className="border-green-200 bg-green-50 shadow-lg">
                    ...
                    <AlertDescription className="text-green-800">
                      ...
                      <br />
                      Your cryptocurrency transfer request has been received and is under review.
                    </AlertDescription>
                  </Alert>
                </div>
              )}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs ${
                          ...
                          {Number(transaction.amount).toFixed(
                            cryptocurrencies.find((c) => c.value === transaction.crypto_type)?.decimals || 8
                          )}{" "}
                          {transaction.crypto_type} to {transaction.wallet_address?.substring(0, 8)}...
                        </p>
                      </div>
                    </div>
                    <Button
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="space-y-1">
              <h2 className="text-3xl font-bold text-gray-900">Cryptocurrency</h2>
              <p className="text-gray-600">Manage your Bitcoin, Ethereum, and USDT</p>
            </div>
            <Button
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="font-medium text-gray-900">{crypto.label}</p>
          ...
          <Card className="animate-in slide-in-from-top duration-300">
            ...
            <CardContent className="space-y-6">
                        ...
                        <SelectItem key={crypto.value} value={crypto.value}>
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full flex items-center justify-center p-1">
                              <img src={crypto.iconUrl} alt={crypto.label} className="w-8 h-8" />
                            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {formData.crypto_type && (
                    <p className="text-xs text-gray-500">
                      ...
                      <span className="font-medium">
                        {formatCryptoAmount(currentBalance, selectedCrypto?.decimals || 8)}
                      </span>{" "}
                      {formData.crypto_type}
                    </p>
                  )}
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-medium">Network Fee (Gas) *</Label>
                  <div className="relative">
                    <Input
                      type="number"
                      step={selectedCrypto ? `0.${"0".repeat(selectedCrypto.decimals - 1)}1` : "0.00000001"}
                      value={formData.gas_fee}
                      onChange={(e) => setFormData({ ...formData, gas_fee: e.target.value })}
                      placeholder={selectedCrypto ? `0.${"0".repeat(selectedCrypto.decimals)}` : "0.00000000"}
                      className="h-12 pr-16"
                    />
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-gray-500">
                  ...
                  {selectedNetwork && (
                    <p className="text-xs text-gray-500">
                      Suggested:{" "}
                      <span className="font-medium">
                        {formatCryptoAmount(Number(selectedNetwork.fee), selectedCrypto?.decimals || 8)}
                      </span>{" "}
                      {formData.crypto_type}
                    </p>
                  )}
              ...
              {/* Label/Note */}
              <div className="space-y-2">
                <Label className="text-sm font-medium">Label / Note (Optional)</Label>
                <Textarea
              ...
              {totalAmount > currentBalance && totalAmount > 0 && (
                <Alert className="border-red-200 bg-red-50">
                  ...
                  <AlertDescription className="text-red-800">
                    ...
                    {formatCryptoAmount(totalAmount, selectedCrypto?.decimals || 8)} {formData.crypto_type} but
                    only have {formatCryptoAmount(currentBalance, selectedCrypto?.decimals || 8)}{" "}
          ...
          <CardContent>
            {cryptoTransactions.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-lg">No cryptocurrency transfers yet</p>
                <p className="text-gray-400 text-sm">Your transfer history will appear here</p>
              </div>
            ) : (
                    ...
                    >
                      <div className="flex-1 space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="font-medium text-gray-900">
                            {transaction.crypto_type} Transfer to {transaction.wallet_address?.substring(0, 10)}...
                          </p>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                          ...
                          <span>
                            ...
                            <span className="font-medium text-gray-800">
                              {formatCryptoAmount(Number(transaction.amount), crypto?.decimals || 8)}
                            </span>{" "}
                          ...
                          <span>
                            Fee:{" "}
                            <span className="font-medium text-gray-800">
                              {formatCryptoAmount(Number(transaction.gas_fee), crypto?.decimals || 8)}
                            </span>{" "}
                            {transaction.crypto_type}
                          </span>
                        </div>
                        ...
                        {transaction.transaction_hash && (
                          <p className="text-xs text-gray-500 font-mono">
                            Hash: {transaction.transaction_hash.substring(0, 20)}...
                          </p>
                        )}
                        <p className="text-xs text-gray-500">
                      ...
                      <div className="flex items-center gap-3 mt-3 sm:mt-0">
                        <div className="text-right">
                          <p className="font-medium text-gray-900">
                            - {formatCryptoAmount(Number(transaction.total_value), crypto?.decimals || 8)}{" "}
                            {transaction.crypto_type}
                          </p>
                        </div>
